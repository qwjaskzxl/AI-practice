...
# 切分数据集
train, val = dataset.split(split_ratio=0.8)
...
for epoch in range(0):
    ...
    if val_acc > best_val:
        torch.save(model.state_dict(), 'results/rnn.pth', _use_new_zipfile_serialization=False)
        best_val = val_acc
...
cnn = CNN().to(device)
rnn = RNN().to(device)
cnn.load_state_dict(torch.load("results/cnn.pth", map_location=torch.device('cpu')))
rnn.load_state_dict(torch.load("results/rnn.pth", map_location=torch.device('cpu')))
...
def predict(text):
    ...
    # 自行实现构建词汇表、词向量等操作
    # 将句子做分词，然后使用词典将词语映射到他的编号
    text2idx = [TEXT.vocab.stoi[i] for i in [w for w in text]] #jb.lcut(text)
    # 转化为Torch接收的Tensor类型
    text2idx = torch.Tensor(text2idx).long()

    # 模型预测部分
    # results = model(text2idx.view(-1, 1))
    results = rnn(text2idx.view(-1, 1)) #+ cnn(text2idx.view(-1, 1))

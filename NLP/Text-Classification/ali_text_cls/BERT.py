...
os.environ['CUDA_VISIBLE_DEVICES'] = '0'  # 要在import torch之前
...
fold_data = all_data2fold(10, 10000)
...
# test
test_data_file = 'dataset/data/test_b.csv'
...
class WordBertEncoder(nn.Module):
    def __init__(self):
        ...
        # with open('dataset/data/bert_emb', 'wb') as f:
...
save_model = 'dataset/saved_dict/bert-back-min5-%d.bin'
save_test = 'results/bert-back-min5.csv'
...
class Trainer():
    def __init__(self, model, vocab):
        ...
        self.dev_data = self.train_data[:20000]  # get_examples(dev_data, model.word_encoder, vocab)
    ...
    def train(self):
        ...
        for epoch in range(1, epochs + 1):
            ...
            if self.best_dev_f1 <= dev_f1:
                ...
                torch.save(self.model.state_dict(), save_model % self.best_dev_f1)
                ...
                if self.early_stop == early_stops:
                    ...
                    break
            self._eval(epoch, test=True)

    def test(self):
        # self.model.load_state_dict(torch.load(save_model)) # bert-min3-97
        self.model.load_state_dict(torch.load('dataset/saved_dict/bert-back-min5-97_.bin'))
        self._eval(self.last_epoch + 1, test=False)
        print('exit!')
        exit()
    ...
    def _eval(self, epoch, test=False):
        ...
        with torch.no_grad():
            for batch_data in data_iter(data, test_batch_size, shuffle=False):
                ...
                batch_inputs, batch_labels = self.batch2tensor(batch_data)
                batch_outputs = self.model(batch_inputs)
                output_all = np.append(output_all, batch_outputs.data.cpu().numpy(), axis=0)
                y_pred.extend(torch.max(batch_outputs, dim=1)[1].cpu().numpy().tolist())
                y_true.extend(batch_labels.cpu().numpy().tolist())
            ...
            if test:
                ...
                if set(y_true) == set(y_pred) and self.report:
                    ...
                    logging.info('\n' + report)
        if test:
            with open('results/probs/%s_result_%.4f.npy' % ('BERT', f1), 'wb') as f:
    ...
    def batch2tensor(self, batch_data):
        ...
        batch_inputs1 = batch_inputs1.cuda()
        batch_inputs2 = batch_inputs2.cuda()

...
if __name__ == '__main__':
    ...
    for i, ckpt in enumerate(ckpts):
        ...
        for set in evaluation_sets:
            accuracy, preds, pred, ans = evaluate(net, loaders[set], args)

            print('Accuracy for ' + set + ' for model ' + ckpt + ":", accuracy)
            # print(classification_report(ans, eval(args.pred_func)(pred)))
            ...
            # Compute
            # set ensembling accuracy
            # Get all ids and answers
            ids = [id for ids, _, _, _ in loaders[set] for id in ids]
            ans = [np.array(a) for _, _, _, ans in loaders[set] for a in ans]
            # for all id, get averaged probabilities
            avg_preds = np.array([np.mean(np.array(ensemble_preds[set][id]), axis=0) for id in ids])  # numpy.ndarray
            # Compute accuracies
            if set != private_set:

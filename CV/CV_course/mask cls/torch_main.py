...
for epoch in range(epochs):
    ...
    for batch_idx, (x, y) in tqdm(enumerate(train_data_loader, 1)):
        x = x.to(device)
        y = y.to(device)
        # writer.add_graph(model=model, input_to_model=x)
        # exit()
        pred_y = model(x)
        ...
        for batch_idx, (x, y) in enumerate(valid_data_loader, 1):
            ...
            acc.append((pred_y==y).sum().item() / x.size(0))
            L.append(criterion(model(x), y).item())
        acc =  np.array(acc).mean()
        L = np.array(loss_list).mean()

        if acc > best_acc:
            print('best val acc:', acc)
            print('min val loss:', L)
            torch.save(model.state_dict(), './results/min_loss.pth', _use_new_zipfile_serialization=False)
            best_acc = acc
        # writer.add_histogram('val_acc', best_acc, it)
        # it += 1
    ...
    torch.save(model.state_dict(), './results/temp-%d.pth'%epoch, _use_new_zipfile_serialization=False)
    print('step:' + str(epoch + 1) + '/' + str(epochs) + ' || Total tra Loss: %.4f' % (np.array(loss_list).mean()))
...
model_path = 'results/min_loss.pth'
# ---------------------------------------------------------------------------

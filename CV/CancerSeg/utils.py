...
# bpe = BPEmb(lang="en", vs=1000, dim=300)  # dim=50 100 200 300都有
...
class BytePairEncoder:
    ...
    def decode(self, ids):
        ...
        # d = bpe.decode_ids([sub_id - self._num_reserved_ids for sub_id in ids])  # 数字没了啊
        # return d if len(d) > 0 else ''


class BpeTextEncoder:
    def __init__(self, vocab_path='/home1/lihaoyuan/data/lip/mvlrs_v1/vocab.1000.subwords.en.t2tsorted'):
...
if __name__ == '__main__':
    ...
    # be = ByteTextEncoder()
    # l = be.encode('lay red by j six now')
    # print(l)
    # s = be.decode(l + [2, 0, 0, 0])
    # print(s)
    #
    # we = WordEncoder(vocab=json.load(open('/home1/lihaoyuan/data/lip/grid/vocab.json')))
    # l = we.encode('lay red by j six now')
    # print(l)
    # s = we.decode(l + [2, 0, 0, 0])
    # print(s)
    bpe = BytePairEncoder()
    l = bpe.encode('lay red by j six now')
    print(l)
    print(bpe.decode(l))
